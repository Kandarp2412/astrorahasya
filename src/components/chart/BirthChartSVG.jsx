...
const DetailMapper = (props) => {
  ...
  const { x, y, detail, signNumber, darkMode } = props;
  ...
  return (
    <>
      {detailArray.map(([key, obj], index) => {
        return (
          <text
            style={{ fill: obj.graha ? "red" : "black", fontSize: "17px", marginTop: "10px" }}
            key={obj.text}
            id="XMLID_77_"
            transform={`translate(${x + 30} ${y + index * 20 - 25})`}
            className="st5 st6 detail"
          >
            {console.log(obj)}
            {console.log(obj.signNumber === 9 ? (obj.text > 6 ? obj.graha : obj.text) : null)}
            {/* {obj.text} */}
            {/* {obj.text > 5 ? ( */}
            {obj.graha ? (
              <>
    ...
    return (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 500" aria-labelledby="123title">
        ...
        <g id="Aries">
          ...
          {/* {obj.signNumber === 1 ? (obj.text > 6 ? obj.graha : obj.text) : null} */}
          {/* <div style={{}}> */}
          <DetailMapper

...
class CustomerApi extends Component {
  ...
  async getCustomers(options) {
    const b = await axios.get("http://localhost:9003/api/profile/get");
    ...
    this.customers = [
      ...b.data.data.map((i) => {
        ...
        return {
          id: i._id,
          address: "",
          avatar: i.profile || "https://i.stack.imgur.com/l60Hf.png",
          createdAt: `${i.date},${i.time},${i.birthPlace}`,
          dateOfBirth: new Date("09/21/1998") || `${i.date} ${i.time} ${i.birthPlace}`,
          email: i.email,
          fullName: i.name,
    ...
    //  console.log(b)
    const queriedCustomers = this.customers.filter((_customer) => {
  ...
  async getCustomerOrders(options) {
    ...
    if (options) {
      ...
      const sortedOrders = applySort(customerOrders, order, orderBy);
